// @fluxstack/live-react - useLiveDebugger Hook
//
...
export function useLiveDebugger(options: UseLiveDebuggerOptions = {}): UseLiveDebuggerReturn {
  const { maxEvents = 500, autoConnect = true, url } = options
    ...
    setComponents(prev => {
      switch (event.type) {
        case 'COMPONENT_MOUNT': {
          ...
          if (prev.find(c => c.componentId === event.componentId)) return prev
          return [...prev, {
            ...
            actionCount: 0, stateChangeCount: 0, errorCount: 0,
          }]
        }
        case 'COMPONENT_UNMOUNT':
          return prev.filter(c => c.componentId !== event.componentId)
        case 'STATE_CHANGE':
          return prev.map(c => c.componentId !== event.componentId ? c : {
            ...c,
        ...
        case 'ACTION_CALL':
          return prev.map(c => c.componentId !== event.componentId ? c : {
            ...c, actionCount: c.actionCount + 1, lastActivity: event.timestamp,
          })
        case 'ACTION_ERROR':
        case 'ERROR':
          return prev.map(c => c.componentId !== event.componentId ? c : {
            ...c, errorCount: c.errorCount + 1, lastActivity: event.timestamp,
          })
        case 'ROOM_JOIN':
          return prev.map(c => {
        ...
        case 'ROOM_LEAVE':
          return prev.map(c => {
            if (c.componentId !== event.componentId) return c
            return { ...c, rooms: c.rooms.filter(r => r !== (event.data.roomId as string)) }
          })
        default:
          return prev.map(c => c.componentId !== event.componentId ? c : { ...c, lastActivity: event.timestamp })
      }
    })
  }, [])
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'DEBUG_DISABLED') { setServerDisabled(true); return }
          if (msg.type === 'DEBUG_WELCOME') {
            setServerDisabled(false)
            const snap = msg.snapshot as DebugSnapshot
            setSnapshot(snap); setComponents(snap.components)
      ...
      ws.onclose = () => {
        setConnected(false); setConnecting(false); wsRef.current = null
        if (serverDisabledRef.current) return
        reconnectTimeoutRef.current = setTimeout(() => { if (autoConnect) connect() }, 3000)
      }
      ws.onerror = () => { setConnecting(false) }
    } catch { setConnecting(false) }
  }, [getWsUrl, maxEvents, autoConnect, updateComponentsFromEvent])
  ...
  const reconnect = useCallback(() => {
    disconnect(); setTimeout(() => connect(), 100)
  }, [connect, disconnect])
  ...
  return {
    connected, connecting, serverDisabled,
    components, events, filteredEvents, snapshot,
    selectedComponentId, selectedComponent,
    selectComponent: setSelectedComponentId,
    filter,
    setFilter: useCallback((partial: Partial<DebugFilter>) => {
