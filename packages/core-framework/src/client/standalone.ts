...
export const startFrontendOnly = async (config: Record<string, unknown> = {}) => {
  ...
  const port = (config.vitePort ?? clientConfig.port ?? 5173) as number
  const host = (config.viteHost ?? clientConfig.host ?? 'localhost') as string
  const logLevel = (config.logLevel || (clientConfig as any).logLevel || 'info') as LogLevel
