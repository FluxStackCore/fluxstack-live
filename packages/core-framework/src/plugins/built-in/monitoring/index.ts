...
import type { FluxStack, PluginContext, RequestContext, ResponseContext, ErrorContext } from "../../../plugins/types"
...
// Static default configuration values - runtime values will be merged from getConfig('monitoring')
const DEFAULTS = {
  enabled: true,
  httpMetrics: true,
  systemMetrics: true,
...
export const monitoringPlugin: Plugin = {
  ...
  onServerStart: async (context: PluginContext) => {
    const options = resolveMonitoringOptions(context as any)
    if (options.enabled) {
      ...
      const metricsRegistry = (context as any).metricsRegistry
      if (metricsRegistry) {
        const version = (context.config as any)?.app?.version ?? '1.0.0'
