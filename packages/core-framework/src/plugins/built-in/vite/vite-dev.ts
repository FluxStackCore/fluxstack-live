...
export async function setupViteDev(context: PluginContext): Promise<void> {
  ...
  const vitePort = (clientConfig.port as number) || 5173
  const viteHost = (clientConfig.host as string) || "localhost"
  const logLevel = ((clientConfig as any).logLevel ?? 'error') as LogLevel
  // Import group logger utilities
  const { endGroup } = await import('../../../utils/logger/group-logger')
