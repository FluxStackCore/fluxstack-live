import type { FluxStack, PluginContext, RequestContext } from "../../../plugins/types"
...
async function proxyToVite(ctx: RequestContext): Promise<void> {
  ...
  const clientCfg = allConfig.client
  const host = clientCfg.host || 'localhost'
  const port = clientCfg.port || 5173
...
export const vitePlugin: Plugin = {
  ...
  setup: async (context: PluginContext) => {
    const allConfig = getConfig()
    const clientCfg = allConfig.client
    const plugins = allConfig.plugins as unknown as Record<string, unknown>

    if (!(plugins.viteEnabled ?? true)) {
    ...
    if (!IS_DEV) {
      ...
      ;(context.app as Record<string, Function>).all('*', createStaticFallback(clientCfg))
      return
    }
  ...
  onServerStart: async (context: PluginContext) => {
    ...
    if (!(plugins.viteEnabled ?? true)) return
    ...
    context.logger.debug(`Vite active - ${clientCfg.host}:${clientCfg.port}`)
  },
  ...
  onBeforeRoute: async (ctx: RequestContext) => {
    ...
    const allConfig = getConfig()
    // viteExcludePaths lives directly on plugins, NOT on plugins.config
    const plugins = allConfig.plugins as unknown as Record<string, unknown>
    const excludePaths = (plugins.viteExcludePaths ?? ['/api', '/swagger']) as string[]

    const shouldSkip = excludePaths.some((prefix: string) =>
