...
export const staticFilesPlugin: Plugin = {
  ...
  setup: async (context: PluginContext) => {
    ...
    const pluginsCfg = (allConfig.plugins?.config ?? {}) as Record<string, unknown>

    if (!(pluginsCfg.staticFilesEnabled ?? true)) {
      context.logger.debug('Static files plugin disabled')
      return
    }

    const projectRoot = process.cwd()
    const publicDir = resolve(projectRoot, (pluginsCfg.staticPublicDir as string) ?? 'public')
    const uploadsDir = resolve(projectRoot, (pluginsCfg.staticUploadsDir as string) ?? 'uploads')
    const cacheMaxAge = pluginsCfg.staticCacheMaxAge
    const enablePublic = pluginsCfg.staticEnablePublic
    const enableUploads = pluginsCfg.staticEnableUploads
