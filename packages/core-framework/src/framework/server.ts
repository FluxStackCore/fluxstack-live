...
import type { FluxStackContext } from "../types"
import type { FluxStack, PluginContext, PluginUtils, PluginConfigSchema, PluginHook } from "../plugins/types"
import { PluginRegistry } from "../plugins/registry"
import { PluginManager } from "../plugins/manager"
import { configProvider, getEnvironmentInfo } from "../config"
import type { FluxStackConfig, DeepPartial } from "../config"
...
export class FluxStackFramework {
  ...
  /** Access typed config from context */
  private get cfg(): FluxStackConfig {
    return this.context.config as FluxStackConfig
  }
  ...
  constructor(config?: DeepPartial<FluxStackConfig>, options?: { app?: any }) {
    // Merge overrides into the global config provider (Laravel-style)
    if (config) {
      configProvider.merge(config)
    }
    // Load the full configuration (defaults + any overrides)
    const fullConfig = configProvider.config
    ...
    // Accept an external Elysia instance to avoid dual-class issues when
    // the framework is loaded from a different node_modules (monorepo dev).
    // The app MUST be provided — we use import type to avoid resolving a
    // second copy of Elysia from the monorepo's node_modules.
    if (!options?.app) {
      throw new Error(
        'FluxStackFramework requires an Elysia instance via options.app. ' +
        'This avoids dual-class issues in monorepo development.'
      )
    }
  ...
  private async initializeAutomaticPlugins() {
    try {
      ...
      const configLoadContext = {
        config: this.context.config as FluxStackConfig,
      ...
      try {
        if (request.method !== 'GET' && request.method !== 'HEAD') {
          ...
          if (contentType?.includes('application/json')) {
            requestContext.body = await request.clone().json().catch(() => undefined) as undefined
          }
        }
      } catch (error) {
  ...
  private async executePluginBeforeRouteHooks(requestContext: { handled?: boolean; response?: Response; [key: string]: unknown }): Promise<Response | null> {
    ...
    for (const pluginName of loadOrder) {
      ...
      if (onBeforeRouteFn && typeof onBeforeRouteFn === 'function') {
        try {
          await onBeforeRouteFn(requestContext as unknown as import('../plugins/types').RequestContext)
  ...
  private async executePluginErrorHooks(errorContext: { handled?: boolean; error: Error; request: Request; [key: string]: unknown }): Promise<Response | null> {
    ...
    for (const pluginName of loadOrder) {
      ...
      if (onErrorFn && typeof onErrorFn === 'function') {
        try {
          await onErrorFn(errorContext as unknown as import('../plugins/types').ErrorContext)
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      for (const pluginName of loadOrder) {
        ...
        const pluginWithRoutes = plugin as FluxStack.Plugin & { plugin?: any }
